...
if __name__ == '__main__':
    ...
    Global_Value.Mode = args.mode
    Global_Value.Process_Num = args.process_num

    print(f'Your input arguments:\n--raw_dataset_path:{Global_Value.Raw_Dataset_file}\n--db_folder_path:{Global_Value.MSA_DB_Path}\n--db_name:{Global_Value.MSA_DB_Name}\n--if_reversed_data:{Global_Value.Is_Use_Reverse_Data}\n--psiblast_threads_num:{Global_Value.Psi_Threads_Num}\n--container_type:{Global_Value.D_or_S}\n--mode:{Global_Value.Mode}\n--process_num:{Global_Value.Process_Num}\n')
    ...
    try:
        ...
        if Global_Value.Mode=='whole' or Global_Value.Mode=='model_only':
            print('Modelling MUT models')
            Prepare_MUT_Models(Table_Path,Res_Table_Name,MUT_PDB_Path)

        if Global_Value.Mode=='whole' or Global_Value.Mode=='blast_only':
            print('Preparing Blast files')
            Prepare_Blast_Files(Table_Path,Res_Table_Name,WT_PSSM_Data_Path,MUT_PSSM_Data_Path,WT_PSI_BLAST_Data_Path,MUT_PSI_BLAST_Data_Path,WT_BLASTP_Data_Path,MUT_BLASTP_Data_Path,Global_Value.MSA_DB_Path,Global_Value.MSA_DB_Name)

        if Global_Value.Mode=='whole' and Global_Value.Is_Use_Reverse_Data:
            print('Adding reverse task')
            Add_Reverse_Data(Table_Path,Res_Table_Name)

        if Global_Value.Mode=='whole':
            Feature_Object_List = []

            print('Beginning features extraction')
            Feature_Extraction(Table_Path,Res_Table_Name,Feature_Object_List,Global_Value.Process_Num)
